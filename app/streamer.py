...
app = FastAPI(title="Border Sentinel Streaming Server")
...
def calculate_direction(history: deque):
    ...
    if abs(dy) < 8 and abs(dx) < 8:
        ...
        return "INBOUND [Approaching]", (0, 0, 255)
    elif dy < -10:
        return "OUTBOUND [Retreating]", (0, 220, 100)
    elif dx > 10:
        return "LATERAL [Eastbound]", (0, 190, 255)
    else:
        return "LATERAL [Westbound]", (0, 190, 255)
...
def generate_frames(cam_id: str):
    ...
    try:
        while True:
            ...
            # Silent socket write with exception handling
            try:
                yield (b'--frame\r\nContent-Type: image/jpeg\r\n\r\n' + buffer.tobytes() + b'\r\n')
            except (ConnectionResetError, BrokenPipeError):
                break
